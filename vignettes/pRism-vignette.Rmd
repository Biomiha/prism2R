...
There are currently three main user-visible functions, the `get_prism_tables` and the `read_prism` functions as well as the `get_analysis_tables` function. The first two are meant to read in the data tables from a GraphPad Prism file. They take either a `.prism` or a `.pzfx` file as input, work out how to parse the data based on the file extension and then return either a vector of table names in the case of `get_prism_tables` or the actual data in the case of the `read_prism` function. The `get_analysis_tables` function on the other hand only works with `.prism` files and attempts to output the main analysis parameters from the `.prism` file. A more detailed explanation below. 
...
*One note of caution with tables specified as character strings - if there are multiple tables with the same name (that is allowed in Prism, or some reason), then you will get an error message reminding you that multiple sheets have been selected.*
...
all_tables <- read_prism(demo_file, sheet = NA)
head(all_tables, n = 3) # First 3 as an example
```
